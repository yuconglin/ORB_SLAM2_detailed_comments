 ...
 * @param[in] vpMatched12
 * 通过词袋模型加速匹配所得到的,两帧特征点的匹配关系所得到的地图点,本质上是来自于候选闭环关键帧的地图点
 * @param[in] bFixScale
 * 当前传感器类型的输入需不需要计算尺度。单目的时候需要，双目和RGBD的时候就不需要了
 */
Sim3Solver::Sim3Solver(KeyFrame *pKF1, KeyFrame *pKF2,
  ...
  // Step 3
  // 将相机坐标系下的三维地图点分别投影到各自相机的二维图像坐标，用于后面和Sim3投影的比较，筛选内点
...
void Sim3Solver::SetRansacParameters(double probability, int minInliers,
  ...
  // 为了计算Sim3,我们需要从这N对匹配点中取三对点;那么如果我有放回的从这些点中抽取三对点,取这三对点均为内点的概率是
  // p0=epsilon^3 相应地,如果取三对点中至少存在一对匹配点是外点, 概率为p1=1-p0
  // 当我们进行K次采样的时候,其中每一次采样中三对点中都存在至少一对外点的概率就是p2=p1^k
  // K次采样中,至少有一次采样中三对点都是内点的概率是p=1-p2
  // 候根据 p2=p1^K 我们就可以导出 K 的公式：K=\frac{\log p2}{\log
  // p1}=\frac{\log(1-p)}{\log(1-epsilon^3)}
  // 也就是说，我们进行K次采样,其中至少有一次采样中,三对点都是内点;
  // 因此我们就得到了RANSAC迭代次数的理论值
  int nIterations;
 ...
 * @param[in] P1
 * 匹配的3D点(三个,每个的坐标都是列向量形式,三个点组成了3x3的矩阵)(当前关键帧)
...
void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2) {
  ...
  // Pr2 对应论文中 r_l,i'，Pr1 对应论文中
  // r_r,i',计算的是P2到P1的Sim3，论文中是left 到 right的Sim3
  cv::Mat M = Pr2 * Pr1.t();
  ...
  // N 矩阵最大特征值（第一个特征值）对应特征向量就是要求的四元数（q0 q1 q2
  ...
  // evec.at<float>(0,0)=q0=cos(theta/2) 这一步的ang实际是theta/2，theta
  // 是旋转向量中旋转角度 ? 这里也可以用 arccos(q0)=angle/2 得到旋转角吧
  double ang = atan2(norm(vec), evec.at<float>(0, 0));

  // vec/norm(vec)归一化得到归一化后的旋转向量,然后乘上角度得到包含了旋转轴和旋转角信息的旋转向量vec
  vec = 2 * ang * vec / norm(vec);  // Angle-axis x. quaternion angle is the
                                    // half
  ...
  if (!mbFixScale) {
    ...
    // Pr1 对应论文里的r_r,i',P3对应论文里的 r_l,i',(经过坐标系转换的Pr2), n=3,
    // 剩下的就和论文中都一样了
