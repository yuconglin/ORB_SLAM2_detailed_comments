...
int main(int argc, char **argv) {
  ...
  // process frames.
  ORB_SLAM2::System SLAM(argv[1],                       // path_to_vocabulary
  ...
  for (int ni = 0; ni < nImages; ni++) {
    ...
    // step 4.6
    // 根据图像时间戳中记录的两张图像之间的时间和现在追踪当前图像所耗费的时间,继续等待指定的时间以使得下一张图像能够
  ...
  // step 7 保存TUM格式的相机轨迹
  // 估计是单目时有尺度漂移, 而LGA GBA都只能优化关键帧使尺度漂移最小,
  // 普通帧所产生的轨迹漂移这里无能为力, 我猜作者这样就只
