 ...
 * @param[out] vTimeStamps
 * 图像序列中每张图像的时间戳(认为左目图像和右目图像的时间戳已经对齐)
 */
...
int main(int argc, char **argv) {
  ...
  cv::FileStorage fsSettings(argv[2],                 // path_to_settings
  ...
  cv::initUndistortRectifyMap(  //计算无畸变和修正转换映射
      ...
      R_l,  //可选的修正变换矩阵,3*3, 从 cv::stereoRectify
            //得来.如果这个矩阵为空矩阵,那么就将会被设置成为单位矩阵
